...
# 数据添加
   1. **数据增强 data augmentation**
      - 修改已有的训练样本，生成新的训练样本。例如修改、扭曲、失真、增加数据噪音
      - OCR（光学符号识别）：放大、缩小、旋转、调整对比度、镜像处理（仅对一些字母适用）、扭曲（distortion）。
...
# 迁移学习 transfer learning
   ...
   - 为什么迁移学习有效?神经网络前几层在训练识别边 角 曲线 基本形状（以手写数字识别与其他图像识别 比如猫、人、车相比），同样适用
   ![alt text](image-13.png)
   - **总结**
     1. 下载具有相同类型（音频、图片等）输入的在大型训练集上预训练的神经网络参数 
     2. 在自己数据上进行进一步训练或微调网络
...
4. **部署与监控** (Deploy in Production)
   ...
   - **持续工作**:
     ...
     - 维护 (Maintain): 随着时间的推移，数据分布可能会发生变化（称为“数据漂移”），导致模型性能下降。如果性能不满足预期，就需要重新训练模型，有时甚至需要回到第二步去收集更多或新的数据。
...
混淆矩阵是所有分类指标的基础，它总结了模型预测结果与真实标签之间的关系。
   ...
   - **假实例** (False Positive, FP): 预测为正，实际为负。（预测错误，误报
   - **假负例** (False Negative, FN): 预测为负，实际为正。（预测错误，漏报
...
我们的目标是，每次划分都让产生的子节点中的数据类别尽可能单一。 
   - **定义**: 熵源于信息论，用于度量一个系统的混乱程度或不确定性。熵越大，代表系统越混乱、越不纯。
   - **公式**: H(p) = -p₁log₂(p₁) - p₀log₂(p₀)，其中 p₁ 是类别1的样本比例
   - **特性**:
     - 当节点完全纯净时（所有样本都属于一个类别，p₁=0 或 p₁=1），熵为 0
     - 当节点完全混乱时（各类别的样本数量相等，p₁=0.5），熵达到最大值 1
   - **定义**: 信息增益衡量的是，在根据某个特征进行划分后，数据的不确定性（熵）减少的程度。
   - **核心逻辑**: 我们选择能让信息增益最大的那个特征作为当前节点的划分标准。信息增益越大，意味着划分后数据的“纯度提升”越多。
   - **公式**: Information Gain = H(父节点) - (w_left * H(左子节点) + w_right * H(右子节点))w_left 和 w_right 是左右子节点样本数量占父节点样本数量的权重。
   - **例子**:
     - 如上图，划分前父节点的熵是 H(0.5) = 1。
     - 按“耳朵形状”划分，信息增益为 1 - [ (5/10)*H(0.8) + (5/10)*H(0.2) ] = 0.28。
     - 按“脸型”划分，信息增益为 0.03。
     - 按“胡须”划分，信息增益为 0.12。
     - 因为 0.28 是最大的，所以我们选择**“耳朵形状”**作为第一个划分特征。
2. **问题二: 何时停止切分**  
为了防止决策树无限生长导致过拟合，需要设定停止条件：
...
决策树不仅可以用于分类，也可以用于回归任务，即预测一个连续的数值。 
  ![predicate a number](image-28.png) 
   ...
   - **逻辑**: 回归树在选择划分特征时，不再是计算“信息增益”，而是计算**“方差减少量 (Variance Reduction)”。它会选择那个能让划分后两个子节点的加权平均方差最小**的特征。
